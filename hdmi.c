...
#define FONT_CHAR_WIDTH 8           // MANTIDO: 8 pixels de largura
#define FONT_CHAR_HEIGHT 24         // NOVO: 8 * 3 = 24 pixels de altura
...
int __not_in_flash("main") main() {
    ...
    while (true) {
        ...
        const char *prompt_msg = "A Entrada ANALOGICA Lida Pelo ADC Foi: ";
        char value_msg[10]; // Buffer para o valor numérico
        sprintf(value_msg, "%d", result);

        // Calcula o comprimento total e a posição inicial
        ...
        int total_len = prompt_len + value_len;
        int start_x = (CHAR_COLS / 2) - (total_len / 2); 
        ...
        for (int i = 0; i < prompt_len; ++i) {
            ...
            // Texto branco, fundo azul escuro
            set_colour(start_x + i, start_y, 0x3f, 0x03); 
        }

        // Escreve a parte do valor numérico em amarelo
        for (int i = 0; i < value_len; ++i) {
            int current_x = start_x + prompt_len + i;
            set_char(current_x, start_y, value_msg[i]);
            // Texto amarelo (0x3C), fundo vermelho escuro (0x30)
            set_colour(current_x, start_y, 0x3C, 0x30); 
